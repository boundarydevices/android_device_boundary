...
KERNEL_OUT=out/target/product/braun/obj/KERNEL_OBJ
mkdir -p $KERNEL_OUT
...
cp $KERNEL_OUT/arch/arm64/boot/Image.gz out/target/product/braun/kernel

if [ "$2" != "m" ]; then
    make -C common O=../$KERNEL_OUT gxl_p230_2g.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    cp -f $KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxl_p230_2g.dtb out/target/product/braun/dtb.img
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image.gz \
        ...
        --ramdisk ${ROOTFS} \
        --second out/target/product/braun/dtb.img \
        --output ./out/target/product/braun/boot.img
    ls -l ./out/target/product/braun/boot.img
    echo "boot.img done"
fi
